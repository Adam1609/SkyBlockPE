...
use thebigsmileXD\SkyBlock\provider\EconomySProvider;
use thebigsmileXD\SkyBlock\provider\PocketMoneyProvider;
use thebigsmileXD\SkyBlock\task\ClearPlotTask;
...
use thebigsmileXD\SkyBlock\provider\DataProvider;
use pocketmine\utils\Config;
use pocketmine\utils\TextFormat;
use pocketmine\level\Level;
use thebigsmileXD\SkyBlock\provider\SQLiteDataProvider;
use thebigsmileXD\SkyBlock\provider\EconomyProvider;
...
class SkyBlock extends PluginBase implements Listener{
	...
	public function getPlotByPosition(Position $position){
		...
		if($x >= 0){
			...
			$X = ceil(($x - $chunks + 1) / $totalSize);
			$difX = abs(($x - $chunks + 1) % $totalSize);
		}
		...
		if($z >= 0){
			...
			$Z = ceil(($z - $chunks + 1) / $totalSize);
			$difZ = abs(($z - $chunks + 1) % $totalSize);
		}
		
		if(($difX > $chunks - 1) or ($difZ > $chunks - 1)){
			return null;
		}
	...
	public function getPlotPosition(Plot $plot){
		...
		return new Position($x, 40, $z, $level);
	}
	...
	public function onEnable(){
		...
		$this->reloadConfig();
		$this->getLogger()->info(TextFormat::GREEN . "Loading the SkyBlock Framework!");
		// $this->getLogger()->warning(TextFormat::YELLOW."It seems that you are running the development build of thebigsmileXD\SkyBlock! Thats cool, but it CAN be very, very buggy! Just be careful when using this plugin and report any issues to".TextFormat::GOLD." http://github.com/wiez/thebigsmileXD\SkyBlock/issues");
